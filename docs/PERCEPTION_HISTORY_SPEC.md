...
3. **summary stats,不是单值**:单值会把波动稀释没(静息心率只剩一个数就没法看基线起伏)。
   每信号每天留**分布 / 分时段 / 分类时长 / 事件列表**等,够细到能看波动。
...
- Tier 2 存的是**已粗化的桶/分布/标签**(与现 `perception_state` 同口径;原始早在端上丢了)。
- 健康/心情等敏感信号的每日记录,at-rest 口径**跟随现有 perception 域**(现状:coarse 明文
  jsonb 存 RDS)。若要更严(enclave-only / 加密 at-rest),与工程师一起决定——**这条留给落地时确认**。
- 永不存:原始样本、逐条时间戳、精确坐标/SSID(端上即弃,历史层也拿不到)。
...
3. 基线窗口:30 天?可配?
4. at-rest 加密口径(§8):跟现状,还是这批敏感历史走更严?
