...
- **no-tool-call fallback**(模型没调工具 → 先 index-fetch 回填;新链路无候选时再由 auto_readside 保底重答一次)。
...
**① no-tool-call 回填(M1.5 必做)**
- prompt-level tool loop 跑完后:模型**输出了 tool_calls** → 执行、用取回的(完成);模型**没输出 tool_calls**(弱模型不按格式 / 判断不需要)→ 进入服务端兜底。
- 服务端兜底内部顺序:
  - 先跑 `memory_index` → selector 选 id → `memory_fetch`;
  - index-fetch 找到候选 → 注入 + **重答一次**(回填);
  - index-fetch 没找到 → 最后退回 `auto_readside`;
  - 两层都没找到 → 不回填(本来就没相关记忆)。
- 这一步**与"模型行不行"无关,纯看实际行为**:会按格式调工具的用工具,不会的先被新 index-fetch 链路兜住;新链路漏掉时再由旧 auto_readside 保底。**任何模型都不丢召回**——这正是 hx 担心的"工具化后弱模型直接失忆"被堵住的地方。

**M1.5 决策树(prompt-level):**
```text
默认挂上 memory 工具(写进 prompt),进 tool loop(max_iters=3)
 ├─ 模型输出 tool_calls → 执行 index/fetch → 用取回的记忆回答 ✅
 └─ 模型没输出 tool_calls → 服务端兜底
          ├─ index-fetch 有候选 → 注入 + 重答(回填)
          ├─ index-fetch 无候选,auto_readside 有候选 → 注入 + 重答(旧链路保底)
          └─ 两层都无候选 → 直接用原答
```
...
> 两个开关语义:**默认对所有模型挂工具(写进 prompt),no-tool-call 时先走 index-fetch 回填,auto_readside 作为最后保底**;prod/test 同一套逻辑,只是灰度比例不同。
...
- 回落场景:先跑 index-fetch;新链路无候选时再跑 auto_readside(§3)。
...
**回落验证(CC 补):** 用一个不支持/弱 tool-use 的模型,确认先走 index-fetch;新链路无候选时自动回落 auto_readside、用户仍能召回。
...
| 风险 | 处理 |
|---|---|
| 模型不发合法 tool_calls | prompt 强约束 + parser 兼容 + **没调工具就先回填 index-fetch,再由 auto_readside 保底**(§2.1) |
