...
V1 当前路径里，`/v1/proactive/tick` 会在 backend 构建 Gate 判定。这个 Gate 会读 identity、memory、最近屏幕帧，再调用系统级模型判断是否触发。它还有一个硬要求：如果模型判定为 true，必须给出一个来自 identity / memory / passive observation 的 `connection.source_id`。
...
1. broadcast 关闭。
2. 系统每 30 分钟创建一次 `heartbeat_broadcast_off` wake。
3. 本体 AI 收到 wake，但拿不到屏幕 plaintext。
4. 它可以直接 sleep。
5. 它也可以根据时间、最近对话、memory、user_state 主动问：
   - “让我看一下？”
   - “你现在在做什么？”
...
| `heartbeat_broadcast_off` | schedule | 不可以 | broadcast 关闭时 30 分钟一次 |
...
1. 这个 wake 不是命令你说话；大多数时候可以 `sleep`。
...
7. `request_broadcast` 要克制。第一版不做平台硬上限，但 prompt 里建议一天最多 2-3 次，实际行为通过 dashboard 观测。
...
第 7 点解释：这里说的“第一版不做机械日上限”，意思是平台先不写死“今天 request_broadcast 超过 3 次就拦截”。原因是这会把人格节制又拉回平台层。第一版先把“少问、一天大概 2-3 次以内”写进默认 Proactive Policy，并在 observability 里记录实际 request 次数。如果 beta 发现 agent 真的频繁骚扰，再加机械上限也不迟。
...
## 20. 当前工程落地状态（2026-06-08）

本轮已经落地：

1. backend `/v1/proactive/tick` 默认创建 V2 wake event，不再调用系统级 Gate LLM。
2. backend tick 不再要求 memory connection，不再因为没有屏幕帧而直接跳过。
3. backend 新增 `/v1/proactive/state`，支持 `user_state`、`ai_state`、`broadcast_state`。
4. backend job schema 记录 `wake_id`、`trigger`、`user_state`、`ai_state`、`broadcast_state`、`agent_action`、`wake_result`。
