...
> 仓 `feedling-mcp`,分支 `test`(cutover 已在 test)。
...
```
> 跳过项:`test_memory_readside_docker_e2e.py`(要 docker)、`test_enclave_route_*`/需 `dstack_sdk` 的 enclave 路测(CI 也不在这跑)。这两类缺环境可跳,不算失败。
...
python -m pytest tests/ -q \
  --ignore=tests/test_memory_readside_docker_e2e.py
```
**通过标准**:全绿,或仅剩"缺 dstack_sdk/docker/网络"的环境性跳过(不是断言失败)。

### 1B. 按区分组跑(定位更清,1A 红了用这个缩小范围)

```bash
# —— genesis / onboarding(闸门①)——
python -m pytest tests/test_genesis_service.py tests/test_genesis_worker.py \
...
python -m pytest tests/test_agent_runtime_discovery.py tests/test_agent_runtime_leases.py \
...
python -m pytest tests/test_agent_runtime_supervisor.py tests/test_genesis_worker.py \
  tests/test_hosted_agent_runtime_cutover.py -v

# —— memory 读写召回 v1(非迁移)——
python -m pytest tests/test_memory_v1_schema.py tests/test_memory_v1_readers.py \
  tests/test_memory_v1_readside.py tests/test_memory_readside.py \
  tests/test_memory_readside_core.py tests/test_memory_index_selector.py \
...
# —— identity ——
python -m pytest tests/test_identity_actions.py tests/test_identity_init_server_encrypt.py \
  tests/test_io_cli_identity.py -v

# —— chat 消费者 / 轮询 ——
python -m pytest tests/test_chat_resident_consumer.py tests/test_chat_resident_self_update.py \
  tests/test_chat_poll_claim_cas.py tests/test_chat_poll_client_release.py -v

# —— proactive / Dream / wake ——
python -m pytest tests/test_proactive_store_v2.py tests/test_proactive_runtime_v2.py \
...
python -m pytest tests/test_perception.py tests/test_perception_history.py \
...
python -m pytest tests/test_multi_tenant_isolation.py tests/test_db.py \
  ...
  tests/test_content_rewrap.py tests/test_enclave_dev_seed.py -v

# —— providers / gateway ——
python -m pytest tests/test_provider_client.py tests/test_providers_manual.py \
  tests/test_model_api_path.py tests/test_model_api_prompts.py tests/test_litellm_gateway.py -v

# —— 杂项 v1 ——
python -m pytest tests/test_copytext.py tests/test_relationship_days.py \
...
```bash
python backend/app.py > /tmp/backend.log 2>&1 &     # 或对 test 部署
python tests/test_api.py <API_URL> --multi-tenant
```
...
```bash
printf 'me: 我家狗叫蛋子\nher: 蛋子今天乖吗？\nme: 上周去了西湖\n' > /tmp/t.txt
python tools/genesis_e2e.py upload --api-url <TEST_API_URL> --register \
  --provider anthropic --model <model_id> --transcript /tmp/t.txt --source-kind history
python tools/genesis_e2e.py verify --api-url <TEST_API_URL> --api-key <key> --job-id <job_id>
```
