...
| `perception.steps(since)` | 时间窗步数 | slow | ⛔ **阻塞**:iOS 无 HealthKit(见 §9) |
| `perception.sleep_last_night()` / `.workout()` / `.vitals()` | 健康趋势 | slow | ⛔ **阻塞**:同上 |
...
| # | 事项 | 性质 | 影响 |
|---|---|---|---|
| B1 | **iOS 无 HealthKit**(零实现:无 import、无 entitlement、权限枚举无 health) | ⛔ 阻塞 | `perception.steps/sleep/workout/vitals` 现在全不可用;catalog 是单边声明。医院例子里的"步数"已改"位置/锚点"(§3.3)规避;健康类等 iOS 补 HealthKit。 |
