...
| B2 | hosted 前台聊天**单次调用** → 要 agentic 多轮 + 后台回灌 | **部分完成(2026-06-21)** | §6;是 D9 能成立的前提。**现状**:`run_tool_loop_v2` + `ToolExecutorV2`(全 catalog perception+memory)已接进 **wake 路**(hosted `wake_consumer`、resident `_process_proactive_jobs`);**但前台聊天两路都未收敛**——hosted `chat_routes._run_model_api_memory_tool_loop` 只认 memory 工具(无 perception.*),resident `_process_messages` 走老单发回复。**违反 D1(应同一引擎)**。已派 Codex(20260621T145308Z):两路收敛到 run_tool_loop_v2 + combined adapters,且守前台**快档预算 + 软交棒**(D17/D9,不能照搬 wake 跑 slow 工具卡住用户)。token 成本由 cost_class 软上限护住。 |
