...
| `perception.weather()` | 粗天气(`condition` / `temperature_bucket` 5℃桶 / `is_daylight`),端上经 WeatherKit 粗化、加密上报 | fast | 🟠 iOS 已建(2026-06-20),**待后端 ingress 接**(见 §9 B1) |
...
| `perception.steps(since)` | 时间窗步数(`step_count_bucket` 500步桶) | slow | 🟠 iOS 已建(2026-06-20),**待后端 ingress 接**(见 §9 B1) |
| `perception.sleep_last_night()` / `.workout()` / `.vitals()` | 健康趋势(睡眠 `asleep_minutes_bucket`30min桶 / 运动 `workout_type`+`duration_min_bucket`+`count_today` / 体征 `resting_heart_rate_bucket`5bpm桶+步数) | slow | 🟠 iOS 已建(2026-06-20),**待后端 ingress 接**(见 §9 B1) |
...
| # | 事项 | 性质 | 影响 |
|---|---|---|---|
| ~~B1~~ | ~~iOS 无 HealthKit(零实现)~~ → **iOS 已补齐(2026-06-20,commit `8bc4504`)** | **依赖**(从 ⛔阻塞 降级) | iOS 已加 HealthKit + WeatherKit entitlement,新增加密信号 `weather` / `health_sleep` / `health_workout` / `health_vitals`,端上分桶后走 v1 加密信封。**剩余前置:① 后端 ingress 接这 4 个信号(见 B1b);② Apple 开发者后台开 HealthKit + WeatherKit capability(真机/TestFlight 才授权成功)。** 见 iOS 仓库 `PERCEPTION_BACKEND_TODO.md`。 |
| B1b(新) | **信号契约对齐:后端 ingress 接 weather/health + 清 focus 死代码** | 缺口(2026-06-20 审出) | iOS 现发的 `weather`/`health_sleep`/`health_workout`/`health_vitals` 在 `ios_contract_v2.py` 里**不属于任何信号集 → `classify_item_v2` 判 `unknown_signal`/`wake_policy=error`**(不是丢,是报错)。要在 V2 契约 + catalog 注册为 encrypted 信号、字段名**逐字对齐** iOS(`.convertToSnakeCase`)、wake policy = **pull_only**(连续信号,§3.1)。另:`focus` iOS 现发真值 `{authorization_status, focused}`,但后端 `operation_observations_v2` 对 focus 返回空、`differ_inputs=()` → 接受即丢;legacy `resolve_focus`/`_apply_focus` 是映射到已删 user_state 的**死代码**,应删,focus 改为纯 pull 在场提示。`HEALTHKIT_UNAVAILABLE_V2` 那张"iOS 未实现"表也已过时,需移除。 |
| B2 | hosted 前台聊天**单次调用** → 要 agentic 多轮 + 后台回灌 | **依赖(升级为 D9 硬前置)** | §6;是 D9 能成立的前提,不是将来优化;有 token 成本(API 用户更贵,已接受 + cost_class 软上限护住)。 |
