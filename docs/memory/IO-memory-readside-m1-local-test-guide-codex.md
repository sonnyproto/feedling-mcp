...
> 当前 test 分支不测这一节：`origin/test` 已移除 MCP 用户线和
> `backend/mcp_server.py`。本节只是保留方案演进记录。本次 test 验收重点是
> route B / API 的 `MEMORY_READSIDE_FOR_MODEL_API` 开关，以及
> `/v1/memory/index`、`/v1/memory/fetch` 后端接口。
