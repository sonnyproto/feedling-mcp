...
def _source_material_paths(source: Any) -> tuple[tuple[str, tuple[str, ...]], ...]:
...
def _source_fixture_bundle(source_path: Path) -> tuple[dict[str, Any], str]:
    ...
    for path, parts in paths:
        digest, size = _hash_regular_material(
    ...
    return source, canonical_json_sha256(bundle)


def verify_source_fixture(persona: GoldenPersona, source_path: Path) -> None:
    source, observed = _source_fixture_bundle(source_path)
