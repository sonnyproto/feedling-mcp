...
6. Agent runs the public skill's Runtime check and establishes Live connection before the user enters Chat. Resident Category A runtimes call `feedling_chat_verify_loop` to verify a reply pipeline; one-shot Category B runtimes stop and tell the operator to configure `feedling-chat-resident` instead of claiming they can poll.
