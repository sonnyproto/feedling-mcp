...
def _memory_readside_available(
    ...
    *,
    ...
    if status in _MEMORY_READSIDE_INACTIVE_STATUSES and status not in {"archived", "superseded"}:
        return False
    if _memory_is_archived(moment) and not include_archived:
        return False
    return True
...
def memory_index():
    ...
    candidates = _memory_readside_candidates(_load_moments(store), store.user_id, limit=limit)
    try:
...
def memory_fetch():
    ...
    by_id = {m.get("id"): m for m in _load_moments(store) if isinstance(m, dict)}
