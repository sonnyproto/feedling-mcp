...
fake_db.init_schema = lambda: None
fake_db.get_config = lambda _key: b"\x01" * 32
fake_db.set_config_if_absent = lambda _key, value: value
fake_db.load_all_users = lambda: []
fake_db.save_all_users = lambda _users: None
...
def client(monkeypatch):
    ...
    monkeypatch.setattr(appmod, "require_user", lambda: store)
    monkeypatch.setattr(appmod, "_extract_api_key", lambda: "key_readside")
...
def test_memory_index_prefilters_top50_and_calls_enclave(client, monkeypatch):
    ...
    monkeypatch.setattr(appmod, "_load_moments", lambda _store: moments)
    captured = {}
    ...
    monkeypatch.setattr(appmod, "_memory_readside_post_enclave", fake_enclave)
