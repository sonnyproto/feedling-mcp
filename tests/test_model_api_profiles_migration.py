...
def _thinking_fallback(user_id: str):
    with db.get_pool().connection() as conn:
        row = conn.execute(
            "SELECT thinking_fallback FROM model_api_routes WHERE user_id = %s",
            (user_id,),
        ).fetchone()
    return row[0] if row else None


def test_backfill_maps_thinking_fallback_true_to_route_column():
    """18dda5b added a per-user thinking_fallback field to the model_api blob
    (parallel to reasoning_effort). The backfill must carry a real JSON boolean
    `true` in the blob into `routes.thinking_fallback IS TRUE`, not the string
    'true' (the blob stores a genuine JSON bool, not text like most other
    fields here)."""
    uid = _uid()
    _seed_blob(uid, {
        "provider": "anthropic",
    ...
    assert _thinking_fallback(uid) is True


def test_backfill_maps_missing_thinking_fallback_to_null():
    uid = _uid()
    _seed_blob(uid, {
        "provider": "anthropic",
        "model": "claude-sonnet-4-5",
    ...
    assert _thinking_fallback(uid) is None
